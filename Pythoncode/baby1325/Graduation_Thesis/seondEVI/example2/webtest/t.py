...
def loss(x1,y1,cond1,x2,y2,cond2,x3,y3,cond3,x4,y4,cond4,x5,y5,cond5,x6,y6,cond6,net):
        ...
        loss_val = 2*(loss_interior)+1*(loss_down+loss_up+loss_left+loss_right)
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig):
    ...
    ax.plot(yc_test.detach().cpu().numpy(),u_real_fig.detach().cpu().numpy().T[500,:], 'b-', linewidth = 2, label = 'Exact')       
    ax.plot(yc_test.detach().cpu().numpy(),u_pred_fig.detach().cpu().numpy().T[500,:], 'r--', linewidth = 2, label = 'Prediction')
    ax.set_xlabel('$y$')
    ax.set_ylabel('$u(0.5,y)$')
    ...
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image1/Possion PINN SAIS x at 0.5.png') 
...
while 0.5*error >2*Posterior_error:
    ...
    while s <= M:
        ...
        # net.to(device)
            ...
            plt.yscale("log")
            plt.legend(prop = {'size':6})
            plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image1/loss32.png")
            plt.close()
            u_pred_fig = u_pred.reshape(h,h)

            plt.figure()
            plt.plot(yc_test.detach().cpu().numpy(),u_real_fig.detach().cpu().numpy().T[500,:], 'b-', linewidth = 2, label = 'Exact')       
            plt.plot(yc_test.detach().cpu().numpy(),u_pred_fig.detach().cpu().numpy().T[500,:], 'r--', linewidth = 2, label = 'Prediction')
            plt.xlabel('$y$')
            plt.ylabel('$u(0.5,y)$')
            # plt.xlim(0,1.1)
            # plt.ylim(-0.5,0.1)
            plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image1/44.png")
            plt.close()
            # 更新学习率
            """ scheduler.step() """
            if i%500==0:
                print("L_2 error is: ",i,float(error_vec))
                print("loss is: ",i,float(loss_1))
    ...
    torch.save(net.state_dict(),"/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/model/modeltest10_{}.pkl".format(1))
    # 计算后验误差指标
