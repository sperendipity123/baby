...
def Q_grad(x1,y1,net):
        ...
        loss_Q_grad=torch.abs(- gradients(u, x1, 2)-gradients(u, y1, 2)+alpha*u-cond3)
        return loss_Q_grad
""" def f_grad(x1,y1,net):
        # 损失函数L1
        # x_interior, y_interior, cond_interior = trainingdata.interior(C_p)
        x1.requires_grad_(True)
        y1.requires_grad_(True)   # need add grad
        ...
        cond2=(200*200* torch.exp(-100* ((x1-0.5)**2+(y1-0.5)**2))*(y1-0.5)*(200*(((x1-0.5)**2+(y1-0.5)**2))-2)-200*400*(y1-0.5)*torch.exp(-100* ((x1-0.5)**2+(y1-0.5)**2))).float().to(device)
        loss_Q_grad=torch.sqrt((-gradients(u, x1, 3)-gradients(gradients(u, y1, 2),x1,1)-cond1)**2+(-gradients(u, y1, 3)-gradients(gradients(u, x1, 2),y1,1)-cond2)**2)
        cond1=((50*4*x1**3-100*3*x1**2-2937.5*2*x1+2987.5)*(x1-0.5)**2+(50*x1**4-100*x1**3-2937.5*x1**2+2987.5*x1-450)*2*(x1-0.5)).float().to(device)
        cond2=((50*4*y1**3-100*3*y1**2-2937.5*2*y1+2987.5)*(y1-0.5)**2+(50*y1**4-100*y1**3-2937.5*y1**2+2987.5*y1-450)*2*(y1-0.5)).float().to(device)
        loss_fx=loss_function((- gradients(u, x1, 3)-gradients(gradients(u, y1, 2),x1,1)+alpha*gradients(u,x1,1)),cond1)
        loss_fy=loss_function((- gradients(u, y1, 3)-gradients(gradients(u, x1, 2),y1,1)+alpha*gradients(u,y1,1)),cond2)
...
def loss(x1,y1,cond1,x2,y2,cond2,x3,y3,cond3,x4,y4,cond4,x5,y5,cond5,x6,y6,cond6,net):
        ...
        """ loss_fx,loss_fy=f_grad(x1,y1,net) """
        loss_val = loss_interior+(loss_down+loss_up+loss_left+loss_right+loss_data_interior)+1/Nc*lossf_qrad
        """ +0.01*loss_fx+0.01*loss_fy """
        # +10**(-4)*loss_grad
        return loss_val
    ...
    #  Omega_F0=sorted0.shape[0]
     ...
     while NFk<=Np:
        ...
        #   SNp=torch.cat([S0[:,0][:,None], S0[:,1][:,None]], dim=1)[idx,:].float().to(device)   # 服从概率pp的Np个初始点
          Sk1=MMA(p,Ns,S0,net,epsilon_r1,loss_Q_grad1)
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig,loss_all,L2_error):
    ...
    # fig.tight_layout()
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image1/Atest.jpg',dpi = 500)
    # 画loss曲线
    plt.figure()
    ax=plt.gca()
    ...
    ax.set_yscale("log")
    ax.legend(prop = {'size':6})
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image1/loss.jpg',dpi = 500)
...
#  给网络输入参数
n=64
layers = np.array([2,n,n,n,n,n,n,1])#5 hidden layers   神经元个数会影响解的误差大小
device="cuda"
...
# start test
u_real = (100*(xx*(xx-1)*(xx-0.5)**4+yy*(yy-1)*(yy-0.5)**4)).float().to(device)
...
while s <= T_max:
    ...
    # 'L-BFGS Optimizer'
    """  if s>470000:
    optimizer = torch.optim.LBFGS(net.parameters(), lr=0.0001, 
                        max_iter = s, 
                        max_eval = None, 
                        tolerance_grad = 1e-05, 
                        tolerance_change = 1e-09, 
    ...
    plt.yscale("log")
    plt.legend(prop = {'size':6})
    plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image/3_loss32.png")
    plt.close()
