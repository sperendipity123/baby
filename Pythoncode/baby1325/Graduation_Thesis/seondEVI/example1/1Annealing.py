...
class trainingdata(nn.Module):
    ...
    def interior(C_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond = 10*(torch.sin(4*np.pi*x)+torch.cos(4*np.pi*y)).float().to(device)
        # print(x)
        # print(y)
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig):
    ...
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(h, cax=cax)
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image1/solve.png')
    ...
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image1/exmple1 x at 0.5.png') 
...
while 0.5*error >2*Posterior_error:
    ...
    while s <= T_max:
        ...
        plt.yscale("log")
        plt.legend(prop = {'size':6})
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image1/moloss32.png")
        plt.close()
        ...
        # plt.xlim(0,1.1)
        # plt.ylim(-0.5,0.1)
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image1/mo44.png")
        plt.close()
        # 更新学习率
        """ scheduler.step() """
        if s%100==0:
        ...
        if s%e ==1 and s>=2:
            ...
            #  fig.colorbar(h, cax=cax)
                ...
                plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image1/mo43.png")
                plt.close()
                ...
                #   y_interior.retain_grad()
                    ...
                    x=Dc[:,0][:,None]
                    y=Dc[:,1][:,None]
                    cond1=10*(torch.sin(4*np.pi*x)+torch.cos(4*np.pi*y)).float().to(device)
    ...
    plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image1/moszj.png")
    torch.save(net.state_dict(),"/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/model/modeltestmo_{}.pkl".format(1))
    # 计算后验误差指标
