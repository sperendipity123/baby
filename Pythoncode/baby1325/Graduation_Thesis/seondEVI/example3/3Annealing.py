...
class trainingdata(nn.Module):
    ...
    def interior(C_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond = (torch.abs(3*x-1)+2*torch.sign(2*y-1)+2*torch.sign(x-0.75)+5*torch.sin(6*np.pi*x)).float().to(device)
        # print(x)
        # print(y)
...
def Q_grad(x1,y1,net):
        ...
        cond3=(torch.abs(3*x1-1)+2*torch.sign(2*y1-1)+2*torch.sign(x1-0.75)+5*torch.sin(6*np.pi*x1)).float().to(device)
...
def loss(x1,y1,cond1,x2,y2,cond2,x3,y3,cond3,x4,y4,cond4,x5,y5,cond5,net):
        ...
        loss_val = 1*(loss_interior)+4*(loss_down+loss_up+loss_left+loss_right)
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig):
    ...
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example3/image/exmple3 x at 0.5.png') 
...
#初始化 数据
epochs = 50000  # 训练代数
e=2000 # 每e个迭代认为损失函数不在下降
h = 100    # 画图网格密度
Nc =2000  # 内点配置点数 500,1000,1500,2000
B_p = 800  # 边界点配置点数
epsilon_p = 0.01
epsilon_r = 1e-23   # 损失函数界，重启指标
p = 0.1
a=0.5
b=1
...
# 方便计算
# savemat('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example3/data/highapprox.mat',{'u_real':u_real.cpu().detach().numpy().ravel(),'u_realx':u_realx.cpu().detach().numpy().ravel(),'u_realy':u_realy.cpu().detach().numpy().ravel()})
# 读取数据
...
# 方便计算
# savemat('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example3/data/Posterior.mat',{'u_real_Posterior':u_real_Posterior.cpu().detach().numpy().ravel(),'u_realx_Posterior':u_realx_Posterior.cpu().detach().numpy().ravel(),'u_realy_Posterior':u_realy_Posterior.cpu().detach().numpy().ravel()})
# 读取数据
...
while 0.5*error >2*Posterior_error:
    ...
    while s <= T_max:
        ...
        plt.yscale("log")
        plt.legend(prop = {'size':6})
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example3/image/moloss32.png")
        plt.close()
        ...
        # plt.xlim(0,1.1)
        # plt.ylim(-0.5,0.1)
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example3/image/mo44.png")
        plt.close()
        # 更新学习率
        """ scheduler.step() """
        if s%100==0:
        ...
        if s%e ==1 and s>=2:
            ...
            #  fig.colorbar(h, cax=cax)
                ...
                plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example3/image/mo43.png")
                plt.close()
                ...
                #   y_interior.retain_grad()
                    ...
                    x=Dc[:,0][:,None]
                    y=Dc[:,1][:,None]
                    cond1=(torch.abs(3*x-1)+2*torch.sign(2*y-1)+2*torch.sign(x-0.75)+5*torch.sin(6*np.pi*x)).float().to(device)
    ...
    plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example3/image/moszj.png")
    torch.save(net.state_dict(),"/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example3/model/modeltestmo_{}.pkl".format(1))
    # 计算后验误差指标
