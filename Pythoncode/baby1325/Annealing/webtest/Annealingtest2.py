     ...
     for j in range(0,nc):
          for m in range (0,ns):
            for k in range(0,2):#二维数据，k表示维度
                ...
                r=min(1,stats.norm.pdf(a)/stats.norm.pdf(q[m,j,k]))
    ...
    #  Omega_F0=sorted0.shape[0]
     ...
     while NFk<=Np:
        ...
        #   SNp=torch.cat([S0[:,0][:,None], S0[:,1][:,None]], dim=1)[idx,:].float().to(device)   # 服从概率pp的Np个初始点
          Sk1=MMA(p,Ns,S0,net,epsilon_r1)
          print(Sk1.shape,Ns)
          loss_Q_grad1=Q_grad(Sk1[:,0][:,None],Sk1[:,1][:,None],net)/torch.sqrt(torch.sum(loss_Q_grad**2))
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig,loss_all,L2_error):
    ...
    # fig.tight_layout()
    plt.savefig('/home/pengyaoguang/baby/Annealing/image/2_Annealingtest.jpg',dpi = 500)
    # 画loss曲线
    plt.figure()
    ax=plt.gca()
    ...
    ax.set_yscale("log")
    ax.legend(prop = {'size':6})
    plt.savefig('/home/pengyaoguang/baby/Annealing/image/2_loss.jpg',dpi = 500)
...
#  给网络输入参数
n=64
layers = np.array([2,n,n,n,n,n,1])#5 hidden layers   神经元个数会影响解的误差大小
device="cuda"
net = PINNNet(layers).to(device)
optimizer = torch.optim.Adam(net.parameters(), lr=0.05,betas=(0.9, 0.999), eps=1e-8, weight_decay=0, amsgrad=False)
...
while s <= T_max:
    ...
    plt.yscale("log")
    plt.legend(prop = {'size':6})
    plt.savefig("/home/pengyaoguang/baby/Annealing/image/2_loss2.png")
    plt.close()
    if s%100==0:
    ...
    if s%e ==1 and s>=2:
         ...
         if PF_SS<epsilon_p:
              ...
              plt.savefig("/home/pengyaoguang/baby/Annealing/image/2.png")
              plt.close()
