...
def loss(x1,y1,cond1,x2,y2,cond2,x3,y3,cond3,x4,y4,cond4,x5,y5,cond5,x6,y6,cond6,net):
        ...
        loss_val = loss_interior+loss_down+loss_up+loss_left+loss_right
        return loss_val

# 定义截断高斯误差
# def truncated_normal(mu, sigma, lower,upper,size):
    
#    #    mu, sigma = 5, 0.7
#    #    lower, upper = mu - 2 * sigma, mu + 2 * sigma  # 截断在[μ-2σ, μ+2σ]     不是标准正态分布的均值和方差需要转化使用
#    x = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma)
#    x1 = np.array(x.rvs(size))

#    y=np.mat(x1).T
#    y=torch.from_numpy(y).float().to(device)
#    return y

# use a= truncated_normal(0,1,-2,2,10)
...
def SAIS(N1, N2, p0, omega, M,net):
    ...
    while k <= M:
        ...
        if k == 1:
            ...
            # x1=truncated_normal(miu_k[0][0].cpu().numpy(),Sigma_k[0][0].cpu().numpy(),-1,1,N1)
            # y1=truncated_normal(miu_k[0][1].cpu().numpy(),Sigma_k[1][1].cpu().numpy(),-1,1,N1)
            # space_to_train = torch.cat([x1,y1],dim=1).float().to(device)  # 选出来的样本点，获得了N1个tensor样本点
            ...
            # x1=truncated_normal(miu_k[0][0].cpu().numpy(),Sigma_k[0][0].cpu().numpy(),-1,1,N1)
            # y1=truncated_normal(miu_k[0][1].cpu().numpy(),Sigma_k[1][1].cpu().numpy(),-1,1,N1)
            # space_to_train = torch.cat([x1,y1],dim=1).float().to(device)   # 获得了N1个tensor样本点
            space_to_train= multivariate_truncated_normal(miu_k.cpu().numpy().ravel(),Sigma_k.cpu().numpy(),[xl,yl],[xr,yr],N1)
            u_shuzhi = net(space_to_train).float().to(device)
            uu = torch.exp(-1000*((space_to_train[:,0][:,None]-0.5)**2+(space_to_train[:,1][:,None]-0.5)**2)).float().to(device)
            loss_fc = torch.abs(u_shuzhi-uu) - epsilon_r
        ...
        # u_shuzhi = net(space_sorted).float().to(device)
        # uu = torch.exp(-10*((space_sorted[:,0][:,None]-0.5)**2+(space_sorted[:,1][:,None]-0.5)**2)).float().to(device)
        # loss_hat = torch.abs(u_shuzhi-uu) - epsilon_r
        # for i in range(0, loss_hat.shape[0]):
        #     if loss_hat[i][0]  <= 0:
        #         loss_hat[i][0] = 0
        sum1 = 0
    ...
    miu_opt = miu_k
    Sigma_opt = Sigma_k
    # xx1=truncated_normal(miu_k[0][0].cpu().numpy(),Sigma_k[0][0].cpu().numpy(),-1,1,N2)
    # yy1=truncated_normal(miu_k[0][1].cpu().numpy(),Sigma_k[1][1].cpu().numpy(),-1,1,N2)
    # space_opt_train=torch.cat([xx1,yy1],dim=1).float().to(device) # 获得了最优估计下的N2个样本点
    ...
    # space_opt_train = np.sqrt(Sigma_opt) * zz1 + miu_opt
    ...
    for i in range(0,D_adaptive.shape[0]):
        """ xx=(D_adaptive[i, 0]-miu_opt[0][0])/np.sqrt(Sigma_opt[0][0])
        yy=(D_adaptive[i, 1]-miu_opt[0][1])/np.sqrt(Sigma_opt[1][1])
        h_optx1=stats.norm.pdf(xx.cpu().numpy(),loc=miu_opt[0][0],scale=np.sqrt(Sigma_opt[0][0]))/np.sqrt(Sigma_opt[0][0])
        h_optx2=stats.norm.cdf((xr-miu_opt[0][0])/np.sqrt(Sigma_opt[0][0]),loc=miu_opt[0][0],scale=np.sqrt(Sigma_opt[0][0]))/np.sqrt(Sigma_opt[0][0])
        h_optx3=stats.norm.cdf((xl-miu_opt[0][0])/np.sqrt(Sigma_opt[0][0]),loc=miu_opt[0][0],scale=np.sqrt(Sigma_opt[0][0]))/np.sqrt(Sigma_opt[0][0])
        h_optx=h_optx1/(h_optx2-h_optx3)
        h_opty1=stats.norm.pdf(yy.cpu().numpy(),loc=miu_opt[0][1],scale=np.sqrt(Sigma_opt[1][1]))/np.sqrt(Sigma_opt[0][0])
        h_opty2=stats.norm.cdf((yr-miu_opt[0][1])/np.sqrt(Sigma_opt[1][1]),loc=miu_opt[0][1],scale=np.sqrt(Sigma_opt[1][1]))/np.sqrt(Sigma_opt[1][1])
        h_opty3=stats.norm.cdf((yl-miu_opt[0][1])/np.sqrt(Sigma_opt[1][1]),loc=miu_opt[0][1],scale=np.sqrt(Sigma_opt[1][1]))/np.sqrt(Sigma_opt[1][1])
        h_opty=h_opty1/(h_opty2-h_opty3)
        sum3=sum3+1/(h_optx*h_opty) """
        ######
        # xx=D_adaptive[i, 0].cpu().numpy()
        # yy=D_adaptive[i, 1].cpu().numpy()
        # xa=(xl-miu_opt[0][0])/np.sqrt(Sigma_opt[0][0])
        # xb=(xr-miu_opt[0][0])/np.sqrt(Sigma_opt[0][0])
        # ya=(yl-miu_opt[0][1])/np.sqrt(Sigma_opt[1][1])
        # yb=(yr-miu_opt[0][1])/np.sqrt(Sigma_opt[1][1])
        # h_optx=stats.truncnorm.pdf(xx,xa,xb,loc=miu_opt[0][0],scale=np.sqrt(Sigma_opt[0][0]))
        # h_opty=stats.truncnorm.pdf(yy,ya,yb,loc=miu_opt[0][1],scale=np.sqrt(Sigma_opt[1][1]))
        # sum3=sum3+1/(h_optx*h_opty)
