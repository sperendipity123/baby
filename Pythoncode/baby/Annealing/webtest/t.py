...
def Q_grad(x1,y1,net):
        ...
        cond3=-2000* torch.exp(-1000* ((x1-0.5)**2+(y1-0.5)**2))*(2000*(((x1-0.5)**2+(y1-0.5)**2))-2).float().to(device)
...
def loss(x1,y1,cond1,x2,y2,cond2,x3,y3,cond3,x4,y4,cond4,x5,y5,cond5,x6,y6,cond6,net):
        ...
        loss_val = 10*(loss_interior+loss_data_interior)+(loss_down+loss_up+loss_left+loss_right)
        # +10**(-4)*loss_grad
        return loss_val
...
def MMA(p,n,S0,net,epsilon_r1,loss_Q_grad):# n是每个维度要多少数据集    p是水平概率，S0是初始种子
     ...
     for j in range(0,nc):
          for m in range(0,ns+1):
               S_new[j*(ns+1)+m,:]=q[m,j,:] +0.001*(2*np.random.random()-1)  
