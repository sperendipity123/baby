...
class trainingdata(nn.Module):
    ...
    def interior(C_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond = -20* torch.exp(-10* ((x-0.5)**2+(y-0.5)**2))*(20*(((x-0.5)**2+(y-0.5)**2))-2).float().to(device)
        # print(x)
        # print(y)
    ...
    def down(B_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond = torch.exp(-10*(9/4+(x-0.5)**2)).float().to(device)
    ...
    def up(B_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond =  torch.exp(-10*(1/4+(x-0.5)**2)).float().to(device)
        return x.requires_grad_(True), y.requires_grad_(True), cond
    ...
    def left(B_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond = torch.exp(-10*(9/4+(y-0.5)**2)).float().to(device)
    ...
    def right(B_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond = torch.exp(-10*(1/4+(y-0.5)**2)).float().to(device)
        return x.requires_grad_(True), y.requires_grad_(True), cond
    ...
    def data_interior(C_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond = torch.exp(-10 * ((x-0.5)**2+(y-0.5)**2)).float().to(device)
        return x.requires_grad_(True), y.requires_grad_(True), cond
...
def l_interior(x1,y1,cond1,net):
        ...
        loss_interior = loss_function(2*(gradients(u,x1,1)*x1+gradients(u,y1,1)*y1)+4*u- gradients(u, x1, 2)-gradients(u, y1, 2), cond1)
...
def Q_grad(x1,y1,net):
        ...
        Q=-gradients(u, x1, 2)-gradients(u, y1, 2)-cond1
        loss_Q_grad=torch.sqrt(gradients(Q, x1, 1)**2+gradients(Q, y1, 1)**2)
        return loss_Q_grad
...
def MMA(p,n,S0,net,epsilon_r1):# n是每个维度要多少数据集    p是水平概率，S0是初始种子
     nc=int(n*p)  #马尔可夫链数,在本文里n=Ns,
     ns=int((1-p)/p) #每条链的状态数
     kk=0
     q=np.zeros((2,S0.shape[0] ,ns+1))
     S_new=np.zeros((2,n ,1))
     SS0=S0.cpu().numpy().ravel().reshape(2,S0.shape[0],1)
     q[:,:,0]=SS0[:,:,0]
     qq1=torch.zeros([1,2]).float().to(device)
     for j in range(0,nc):
          for m in range (0,ns):
            for k in range(0,2):#二维数据，k表示维度
                a=q[k,j,m]+np.random.random()   
                r=min(1,stats.norm.pdf(a)/stats.norm.pdf(q[k,j,m]))
                if np.random.random()<r:
                     qq1[0,k]=a
                else:
                     qq1[0,k]=q[k,j,m]   
            loss_Q_grad=Q_grad(qq1[0,0].reshape(1,1),qq1[0,1].reshape(1,1),net)
            qq2=qq1.cpu().numpy().ravel().reshape(2,1,1)
            if loss_Q_grad> epsilon_r1:
                    q[:,j,m+1]=qq2[:,0,0]
            else:
                    q[:,j,m+1]=q[:,j,m] 
     for j in range(0,nc):
          for m in range(0,ns+1):
               S_new[:,(j-1)*(ns+1)+m,0]=q[:,j,m]   
     S_new=torch.from_numpy(S_new.squeeze().reshape(n,2)).float().to(device)                     
     return S_new
...
def Subset_simulation(omega,elta,Nc,p,net):
    ...
    #  u_shuzhi = net(space_to_train_0).float().to(device)
     ...
     while NFk<=Np:
        ...
        #   PP=Omega_Fk1.shape[0]/Omega_Fk0
          ...
          Sk1=MMA(p,Ns,SNp,net,epsilon_r1)
          loss_Q_grad1=Q_grad(Sk1[:,0][:,None],Sk1[:,1][:,None],net)/loss_Q_grad
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig,loss_all,L2_error):
    ...
    # fig.tight_layout()
    plt.savefig('/home/pengyaoguang/baby/Annealing/image/Annealingtest.jpg',dpi = 500)
    # 画loss曲线
    plt.figure()
    ax=plt.gca()
    ...
    ax.set_yscale("log")
    ax.legend(prop = {'size':6})
    plt.savefig('/home/pengyaoguang/baby/Annealing/image/loss.jpg',dpi = 500)
...
# start test
u_real =  torch.exp(-10*((xx-0.5)**2 +(yy-0.5)**2)).float().to(device)
...
while s <= T_max:
    ...
    plt.yscale("log")
    plt.legend(prop = {'size':6})
    plt.savefig("/home/pengyaoguang/baby/Annealing/image/loss2.png")
    plt.close()
    if s%10==0:
    ...
    if s%e ==1 and s>=2:
         ...
         if PF_SS<epsilon_p:
              ...
              plt.figure()
              plt.plot(Dc[:,0].cpu().detach().numpy(),Dc[:,1].cpu().detach().numpy(),'o')
              plt.savefig("/home/pengyaoguang/baby/Annealing/image/1.png")
              plt.close()
            ...
            #   y_interior.retain_grad()
              ...
              x=Dc[:,0][:,None]
              y=Dc[:,1][:,None]
              cond1 = -20* torch.exp(-1000 * ((x-0.5)**2+(y-0.5)**2))*(2000*(((x-0.5)**2+(y-0.5)**2))-2).float().to(device)
              cond1.requires_grad_(True)
              cond_interior =cond1
              cond2 = torch.exp(-10 * ((x-0.5)**2+(y-0.5)**2)).float().to(device)
