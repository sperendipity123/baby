...
class pdeloss2(nn.Module):
    ...
    def loss2(x1,y1,cond1,x2,y2,cond2,x3,y3,cond3,x4,y4,cond4,x5,y5,cond5,Rnet,net):
            ...
            loss_val1 = lambdaf_new*(loss_interior1)+lambdab_new*(loss_down1+loss_up1+loss_left1+loss_right1)
    ...
    def closure2():
        ...
        loss_2 = pdeloss2.loss2(x_interior1, y_interior1, cond_interior1,x_down1, y_down1, cond_down1,x_up1, y_up1, cond_up1, x_left1, y_left1, cond_left1,x_right1, y_right1, cond_right1,Rnet,model)
...
def fnet_single(params, x):
    return functional_call(Rnet, params, (x.unsqueeze(0),)).squeeze(0)  # 不改变网络本身的状态下暂时性提供参数,squeeze(0)降维,unsqueeze(0)升维
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig):
    ...
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(h, cax=cax)
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/imagetest/abs error.png')
...
while error >gamma_0**2+h1**2+Posterior_error:
    ...
    for i in range(T_max):
            ...
            plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/imagetest/dpinnloss32.png")
            plt.close()
            ...
            # plt.xlim(0,1.1)
            # plt.ylim(-0.5,0.1)
            plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/imagetest/dpinn44.png")
            plt.close()
    torch.save(net.state_dict(),"/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/model/PINN_modeltestortest2_2.pkl")
    model=PINNNet(layers).to(device)
    ...
    # 定义优化器和学习率
    optimizer1 = torch.optim.Adam(Rnet.parameters(), lr=0.001,betas=(0.9, 0.999), eps=1e-8, weight_decay=0, amsgrad=False)
    params = {k: v.detach() for k, v in Rnet.named_parameters()}
    xf_train=torch.cat([x_interior1, y_interior1],dim=1).float().to(device)
    ...
    xb_train=torch.cat([torch.cat([x_down1,x_up1,x_left1,x_right1],dim=0),torch.cat([y_down1,y_up1,y_left1,y_right1],dim=0)],dim=1).float().to(device)
    ...
    for i in range(T_max2):
        ...
        # net.to(device)
        u_pred = (model(xy11_test)+Rnet(xy11_test)).float().to(device)
        ...
        plt.yscale("log")
        plt.legend(prop = {'size':6})
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/imagetest/dnewloss32.png")
        plt.close()
        u_pred_fig = u_pred.reshape(h,h)
        ...
        # plt.xlim(0,1.1)
        # plt.ylim(-0.5,0.1)
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/imagetest/dnewpinn44.png")
        plt.close()
    # 计算后验误差指标
    u_pred_Posterior =(model(torch.cat([xx_Posterior,yy_Posterior],dim=1))+Rnet(torch.cat([xx_Posterior,yy_Posterior],dim=1))).float().to(device)
...
u_pred =model(xy11_test)+Rnet(xy11_test)
u_pred_fig = u_pred.reshape(h,h)
ax3.plot_surface(xm_test.detach().cpu().numpy(),ym_test.detach().cpu().numpy(),u_pred_fig.detach().cpu().numpy(),cmap='rainbow')
plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/imagetest/dPINNszj.png")
