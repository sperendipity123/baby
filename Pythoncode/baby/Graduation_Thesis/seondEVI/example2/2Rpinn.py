...
model.load_state_dict(torch.load("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/model/PINN_modeltestortest2_2.pkl"))
# 新的网络
...
""" def Lf(params,x,model):
     x.requires_grad_(True)
     u=model(x)
     u_epsilon=fnet_single(params,x)
     return - gradients(u_epsilon+u, x[:,0], 2)-gradients(u_epsilon+u, x[:,1], 2)+alpha*(u_epsilon+u)
def Lc(params,x,model):
     u=model(x)
     u_epsilon=fnet_single(params,x)
     Ld=-gradients(u_epsilon+u,x[0:B_p-1,1],1)+g*psi_t(u_epsilon+u,epsilon)
     Lu=gradients(u_epsilon+u,x[B_p:2*B_p-1,1],1)+g*psi_t(u_epsilon+u,epsilon)
     Ll=-gradients(u_epsilon+u,x[2*B_p:3*B_p-1,0],1)+g*psi_t(u_epsilon+u,epsilon)
     Lr=gradients(u_epsilon+u,x[3*B_p:4*B_p-1,0],1)+g*psi_t(u_epsilon+u,epsilon)
     return Ld+Lu+Ll+Lr """
# 计算NTK
...
for i in range(T_max):
        if i%e ==1 and s>=2:
            lambdaf=(sum(lambda_f)+sum(lambda_b))/sum(lambda_f)
            lambdab=(sum(lambda_f)+sum(lambda_b))/sum(lambda_b)
        ...
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/imagetest/loss32.png")
        plt.close()
        ...
        # plt.xlim(0,1.1)
        # plt.ylim(-0.5,0.1)
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/imagetest/44.png")
        plt.close()
...
# plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/imagetest/szj.png")
