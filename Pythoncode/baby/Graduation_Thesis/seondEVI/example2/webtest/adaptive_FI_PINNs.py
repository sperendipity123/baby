...
# Data Prep
data = scipy.io.loadmat('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/data/example2.mat')  # note change
D=data['D'].astype(float)
E=data['E'].astype(float)
u0=data['u0'].astype(float)
bhgs=data['bhgs']
bjd=data['bjd'].astype(float)
Dbjd=data['Dbjd'].astype(float)
space=data['space']
space=space[0][0]
...
# bjd=torch.from_numpy(bjd).float().to(device)
# Dbjd=torch.from_numpy(Dbjd).float().to(device)
...
def high_approximate(x,y,u0,space):
    ...
    for idx in range(0,x.shape[0]):
         uh[idx][0]=choose_traingle(x[idx][0].cpu().item(),y[idx][0].cpu().item(),u0,space)
    u=uh.float().to(device)
    return u
...
def psi_t(t,epsilon):
     return torch.where(t>=epsilon,1,torch.where(torch.abs(t)<=epsilon,t/epsilon,-1))
...
class trainingdata(nn.Module):
    ...
    """ def data_interior(C_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond = high_approximate(x,y,u0,space).float().to(device)
        return x.requires_grad_(True), y.requires_grad_(True), cond """
...
def l_abs(x1,y1,net):
        ...
        uxy = torch.cat([x1, y1], dim=1).float().to(device)
        u = net(uxy).float().to(device)
        cond1=((50*x1**4-100*x1**3-2937.5*x1**2+2987.5*x1-450)*(x1-0.5)**2+(50*y1**4-100*y1**3-2937.5*y1**2+2987.5*y1-450)*(y1-0.5)**2).float().to(device)
        loss_interior_abs=torch.abs(- gradients(u, x1, 2)-gradients(u, y1, 2)+alpha*u-cond1)
...
def l_down(x2,y2,cond2,net):
        ...
        u = net(uxy)
        loss_down = loss_function(-u*gradients(u,y2,1)+g*psi_t(u,epsilon), cond2)
        return loss_down
...
def l_up(x3,y3,cond3,net):
        ...
        u = net(uxy)
        loss_up = loss_function(u*gradients(u,y3,1)+g*psi_t(u,epsilon), cond3)
        return loss_up
...
def l_left(x4,y4,cond4,net):
        ...
        u = net(uxy)
        loss_left = loss_function(-u*gradients(u,x4,1)+g*psi_t(u,epsilon), cond4)
        return loss_left
...
def l_right(x5,y5,cond5,net):
        ...
        u = net(uxy)
        loss_right = loss_function(u*gradients(u,x5,1)+g*psi_t(u,epsilon), cond5)
        return loss_right
...
def SAIS(N1, N2, p0, omega, M,net):
    ...
    while k <= M:
        ...
        if k == 1:
            ...
            """ uu=high_approximate(space_to_train[:,0][:,None],space_to_train[:,1][:,None],u0,space).float().to(device)
    ...
    """ uu=high_approximate(space_opt_train[:,0][:,None],space_opt_train[:,1][:,None],u0,space).float().to(device)
...
""" def solutionplot(u_pred_fig,u_error_fig,u_real_fig):
    ...
    h = ax.imshow((u_pred_fig).detach().cpu().numpy(), interpolation='nearest', cmap='rainbow', 
                extent=[xmm.detach().cpu().numpy().min(), xmm.detach().cpu().numpy().max(), ymm.detach().cpu().numpy().min(), ymm.detach().cpu().numpy().max()], 
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig):
    ...
    ax.plot_surface(xmm.cpu().detach().numpy(), ymm.cpu().detach().numpy(), u_pred_fig.cpu().detach().numpy(),alpha=0.9,cmap='rainbow')
    fig.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image/example2_1The numerical solution.jpg")

    # 作真解图
    fig = plt.figure()   # 创建一个空的画布
    ax = fig.add_axes(Axes3D(fig))   #添加轴
    ax.plot_surface(xmm.cpu().detach().numpy(), ymm.cpu().detach().numpy(), u_real_fig.cpu().detach().numpy(),alpha=0.9,cmap='rainbow')
    # ax.text2D(0.5, 0.9, "real solve", transform=ax.transAxes)
    # plt.show()
    fig.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image/example2_1High approximate solution.jpg")
    ...
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(h, cax=cax)
    plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example2/image/example2_1Abs error.jpg")
...
while s <= M:
    ...
    if P_f < epsilon_p:
        break
         ...
         x=D_adaptive[:,0][:,None]
         y=D_adaptive[:,1][:,None]
         cond1 = 10*(torch.sin(4*torch.pi*x)+torch.cos(4*torch.pi*y)).float().to(device)
         ...
         cond2 = high_approximate(x,y,u0,space).float().to(device)
...
xmm, ymm = torch.meshgrid(xc, yc)
xx = xmm.reshape(-1, 1)   #转成一列
yy = ymm.reshape(-1, 1)
xy = torch.cat([xx, yy], dim=1).float().to(device)

# start test
u_real = high_approximate(xx.float().to(device),yy.float().to(device),u0,space).float().to(device)
u_pred = net(xy)
