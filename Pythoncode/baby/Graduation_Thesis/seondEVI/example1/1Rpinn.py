...
def l_interior(x1,y1,cond1,net,model):
        # 损失函数L1
        uxy = torch.cat([x1, y1], dim=1)
        u_epsilon = net(uxy).float().to(device)
        u=model(uxy).float().to(device)
        cond =10*(torch.sin(4*np.pi*x1)+torch.cos(4*np.pi*y1)).float().to(device)
...
def l_down(x2,y2,cond2,net,model):
        # 损失函数L2
...
def TV(x2,y2,net,model):
        ...
        KTV=torch.sqrt(torch.linalg.norm((gradients(u_epsilon+u,x2,1)), 2)**2+torch.linalg.norm((gradients(u_epsilon+u,y2,1)), 2)**2)
        
        return KTV


def l_up(x3,y3,cond3,net,model):
        # 损失函数L3
...
def l_left(x4,y4,cond4,net,model):
        # 损失函数L4
...
def l_right(x5,y5,cond5,net,model):
        # 损失函数L5
...
def loss(x1,y1,cond1,x2,y2,cond2,x3,y3,cond3,x4,y4,cond4,x5,y5,cond5,net,model):
        # all the loss loss(x1,y1,cond1,x2,y2,cond2,x3,y3,cond3,x4,y4,cond4,x5,y5,cond5,x6,y6,cond6,net):
        loss_interior= l_interior(x1,y1,cond1,net,model)
        loss_down = l_down(x2,y2,cond2,net,model)
        loss_up = l_up(x3,y3,cond3,net,model)
        loss_left = l_left(x4,y4,cond4,net,model)
        loss_right = l_right(x5,y5,cond5,net,model)
        KTV_i=TV(x1,y1,net,model)
        KTV_d=TV(x2,y2,net,model)
        KTV_u=TV(x3,y3,net,model)
        KTV_l=TV(x4,y4,net,model)
        KTV_r=TV(x5,y5,net,model)
        # KTV_d=TV(x2,y2,net,model)
        """ loss_data_interior = l_data(x6,y6,cond6,net)
        lossf_qrad=f_qrad(x1,y1,net) """
        loss_val = lambdaf_new*(loss_interior)+lambdab_new*(loss_down+loss_up+loss_left+loss_right)
        # +1*KTV_i+1*(KTV_d+KTV_u+KTV_l+KTV_r)
...
def closure():
    ...
    loss_1 = loss(x_interior, y_interior, cond_interior,x_down, y_down, cond_down,x_up, y_up, cond_up, x_left, y_left, cond_left,x_right, y_right, cond_right,net,model)
    optimizer.zero_grad()
...
""" def closure():
    ...
    loss_1 = loss1(x_interior1, y_interior1, cond_interior1,x_down1, y_down1, cond_down1,x_up1, y_up1, cond_up1, x_left1, y_left1, cond_left1,x_right1, y_right1, cond_right1,net1,net2,alpha1)
    loss_2 = loss2(x_interior2, y_interior2, cond_interior2,x_down2, y_down2, cond_down2,x_up2, y_up2, cond_up2, x_left2, y_left2, cond_left2,x_right2, y_right2, cond_right2,net1,net2,alpha2)          
    
    lossall=omega1*loss_1+omega2
    # loss_1.backward()  loss_1 = loss(x_interior, y_interior, cond_interior,x_down, y_down, cond_down,x_up, y_up, cond_up, x_left, y_left, cond_left,x_right, y_right, cond_right,x_data_interior, y_data_interior, cond_data_interior,net)  
    optimizer.zero_grad()
    lossall.backward()
    return lossall """
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig):
    ...
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(h, cax=cax)
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/imagetest/solve.png')
    ...
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(h, cax=cax)
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image2/abs error.png')
    ...
    plt.savefig('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image2/example1 x at 0.5.png') 
...
model.load_state_dict(torch.load("Graduation_Thesis/seondEVI/example1/model/PINN_modeltestortest2_2.pkl"))
""" params_model= {k: v.detach() for k, v in model.named_parameters()}
def fmodel_single(params_model, x):
    return functional_call(model, params_model, (x.unsqueeze(0),)).squeeze(0)  # 不改变网络本身的状态下暂时性提供参数,squeeze(0)降维,unsqueeze(0)升维 """
# 新的网络
net = Newnet(layers2).to(device)
# 参数
params = {k: v.detach() for k, v in net.named_parameters()}
def fnet_single(params, x):
    return functional_call(net, params, (x.unsqueeze(0),)).squeeze(0)  # 不改变网络本身的状态下暂时性提供参数,squeeze(0)降维,unsqueeze(0)升维
""" def Lf_single(params,x):
     x.requires_grad_(True)
     u_epsilon=fnet_single(params,x)
     grad(grad(fnet_single)(dict(params),(x.unsqueeze(0),)).squeeze(0))
     return - vjp(u_epsilon, x[0], 2)-gradients(u_epsilon, x[1], 2)+alpha*(u_epsilon)
def Lc_single(params,x):
     u_epsilon=fnet_single(params,x)
     Ld=-gradients(u_epsilon,x[0:B_p-1,1],1)+g*psi_t(u_epsilon,epsilon)
     Lu=gradients(u_epsilon,x[B_p:2*B_p-1,1],1)+g*psi_t(u_epsilon,epsilon)
     Ll=-gradients(u_epsilon,x[2*B_p:3*B_p-1,0],1)+g*psi_t(u_epsilon,epsilon)
     Lr=gradients(u_epsilon,x[3*B_p:4*B_p-1,0],1)+g*psi_t(u_epsilon,epsilon)
     return Ld+Lu+Ll+Lr
def compute_loss(params,x):
     x.requires_grad_(True)
     y=functional_call(net, params, x)
     cond=gradients(model(x),x[0],2)+gradients(model(x),x[1],2)-alpha*model(x)
     return loss_function(-gradients(y,x[0],2)-gradients(y,x[1],2)+alpha*y,cond) """
# 计算NTK
def empirical_ntk_jacobian_contraction(fnet_single, params, x1, x2, compute='full'):
    # Compute J(x1)
    jac1 = vmap(jacrev(fnet_single), (None, 0))(params, x1)  # jacrev 计算jakobi
    jac1 = jac1.values()
...
""" optimizer = torch.optim.Adam(params=chain(net1.parameters(),net2.parameters()), lr=0.001,betas=(0.9, 0.999), eps=1e-8, weight_decay=0, amsgrad=False) """
...
alpha2=0.4
h = 100    # 画图网格密度
Nc =2000  # 内点配置点数 500,1000,1500,2000
...
# 读取数据
data_real = scipy.io.loadmat('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/data/highapprox.mat')  # note change
...
# 读取数据
data_Posterior = scipy.io.loadmat('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/data/Posterior.mat')  # note change
...
# 计算权重
xf_train=torch.cat([x_interior, y_interior],dim=1).float().to(device)
fsum= empirical_ntk_jacobian_contraction(fnet_single, params, xf_train, xf_train,'trace')
lambda_f, eigvec_f = np.linalg.eig(fsum.detach().cpu().numpy())
xb_train=torch.cat([torch.cat([x_down,x_up,x_left,x_right],dim=0),torch.cat([y_down,y_up,y_left,y_right],dim=0)],dim=1).float().to(device)
bsum= empirical_ntk_jacobian_contraction(fnet_single, params, xb_train, xb_train, 'trace')
lambda_b, eigvec_b = np.linalg.eig(bsum.detach().cpu().numpy())
miuu=0.9
e=5
lambdaf_new=1
lambdab_new=1
for i in range(T_max):
        if i%e ==1 and s>=2:
            lambdaf=(np.sum(lambda_f)+np.sum(lambda_b))/np.sum(lambda_f)
            lambdab=(np.sum(lambda_f)+np.sum(lambda_b))/np.sum(lambda_b)
            lambdaf_new=(1-miuu)*lambdaf_new+miuu* lambdaf
            lambdab_new=(1-miuu)*lambdab_new+miuu* lambdab
        ...
        # net.to(device)
        u_pred = (net(xy11_test)+model(xy11_test)).float().to(device)
        error_vec=torch.linalg.norm((u_real - u_pred), 2) / torch.linalg.norm(u_real,2)
        L2_error.append(error_vec.cpu().detach().numpy())
        plt.figure()
        plt.plot(range(len(loss_all)),loss_all,'b-', linewidth = 2, label = 'Loss1')
        plt.plot(range(len(L2_error)),L2_error,'r--', linewidth = 2, label = 'L2 error')
        plt.xlabel('$epochs$')
        plt.yscale("log")
        plt.legend(prop = {'size':6})
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/imagetest/loss32.png")
        plt.close()
        ...
        # plt.xlim(0,1.1)
        # plt.ylim(-0.5,0.1)
        plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/imagetest/44.png")
        plt.close()
...
u_pred =net(xy11_test)+model(xy11_test)
u_pred_fig = u_pred.reshape(h,h)
ax3.plot_surface(xm_test.detach().cpu().numpy(),ym_test.detach().cpu().numpy(),u_pred_fig.detach().cpu().numpy(),cmap='rainbow')
plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/imagetest/szj.png")
...
# 计算后验误差指标
u_pred_Posterior =(net(torch.cat([xx_Posterior,yy_Posterior],dim=1))+model(torch.cat([xx_Posterior,yy_Posterior],dim=1))).float().to(device)
