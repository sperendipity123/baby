...
# Data Prep
data = scipy.io.loadmat('/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/data/example1.mat')  # note change
...
class trainingdata(nn.Module):
    ...
    def interior(C_p):
        ...
        x=x.float().to(device)
        y=y.float().to(device)
        cond = 10*(torch.sin(4*torch.pi*x)+torch.cos(4*torch.pi*y)).float().to(device)
        # print(x)
        # print(y)
...
def l_abs(x1,y1,net):
        ...
        uxy = torch.cat([x1, y1], dim=1).float().to(device)
        u = net(uxy).float().to(device)
        cond1=10*(torch.sin(4*torch.pi*x1)+torch.cos(4*torch.pi*y1)).float().to(device)
...
def solutionplot(u_pred_fig,u_error_fig,u_real_fig):
    ...
    fig.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image/4The numerical solution.jpg")
    ...
    # plt.show()
    fig.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image/4High approximate solution.jpg")
    ...
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(h, cax=cax)
    plt.savefig("/home/pengyaoguang/baby/Graduation_Thesis/seondEVI/example1/image/4Abs error.jpg")
